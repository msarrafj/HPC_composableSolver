#!/bin/bash
# When running remember: I) for TET: DG (nx=10 => 192000), CG ( nx=28 => 195112), RT0 (nx=17 => 180336)
#                       II) for HEX: DG (nx=15 => 216000), CG ( nx=29 => 216000), RT0 (nx=29 => 200158)
# file.py changes
...
for k in "${solver_type[@]}"
  ...
  for j in "${element[@]}"
    do
    echo "element is: $j"
	mpiexec -n 1 python ../../3D_composable_solvers_RT0.py 1e-7 1 $k hypre 29 $j
     for l in "${MPI[@]}"
      do
      echo "# MPI proc is: ${l}"
