...
Formulation: Stabilized mixed DG formulation 
...
Run as:
  ...
  <solver_type> = naive (1), split by fields (2), split by scales (3)
...
#penalty parameter
eta_u, eta_p = Constant(10.), Constant(10.)
...
if element == 'TET':
	...
	- div(w1) * p1 *dx - div(w2) * p2 * dx \
	    ...
	    avg(q1) * jump(v1,n) * dS -\
	    avg(q2) * jump(v2,n) * dS +\
	    eta_u * h_avg * avg(alpha1) * (jump(w1,n) * jump(v1,n)) * dS +\
	    eta_u * h_avg * avg(alpha2) * (jump(w2,n) * jump(v2,n)) * dS +\
	...
	a = dot(w1, alpha1*v1)*dx + dot(w2, alpha2*v2)*dx \
	    ...
	    0.5 * dot( alpha2 * w2 - grad(q2), \
	               invalpha2 * (alpha2 * v2 + grad(p2)) ) * dx +\
	    jump(w1,n) * avg(p1) * (dS_h + dS_v) +\
	    jump(w2,n) * avg(p2) * (dS_h + dS_v) -\
	    avg(q1) * jump(v1,n) * (dS_h + dS_v) -\
	    avg(q2) * jump(v2,n) * (dS_h + dS_v) +\
	    eta_u * h_avg * avg(alpha1) * (jump(w1,n) * jump(v1,n)) * (dS_h + dS_v) +\
	    eta_u * h_avg * avg(alpha2) * (jump(w2,n) * jump(v2,n)) * (dS_h + dS_v) +\
	    (eta_p / h_avg) * avg(1 / alpha1) * dot(jump(q1,n),jump(p1,n)) * (dS_h + dS_v) +\
	    (eta_p / h_avg) * avg(1 / alpha2) * dot(jump(q2,n),jump(p2,n)) * (dS_h + dS_v)
...
#file = File('Output/v1_3D_DG.pvd')
#file.write(v1sol)

#file = File('Output/v2_3D_DG.pvd')
#file.write(v2sol)

#file = File('Output/p1_3D_DG.pvd')
#file.write(p1sol)

#file = File('Output/p2_3D_DG.pvd')
#file.write(p2sol)
